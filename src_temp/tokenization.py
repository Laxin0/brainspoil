    ...
    def get_tokens(self):
        ...
        while i < len(self.src):
            if self.src[i] == ';':
                ...
                tokens.append(Token(TokenType.CUR_CL, None, (lc, cc)))
                i += 1
                cc += 1
            elif self.src[i] == '{':
                tokens.append(Token(TokenType.CUR_OP, None, (lc, cc)))
                i += 1
                cc += 1
            elif self.src[i] == '\'':
                i += 1
                ...
                cc += 1
                if self.src[i] != '\'':
                    print(f'[ERROR] at {(ls, cc)}: expected closing `\'`, but found `{self.src[i]}` char')
                    exit(1)
                i += 1
                cc += 1
                tokens.append(Token(TokenType.INTLIT, ord(char), loc))
